...
set_time_limit(0);
$url = "http://www.tech-universal.eu/comments/feed/";
...
class SerialTv extends ListFetcher {
    ...
    function __construct($url) {
        ...
        if ($data && preg_match('/http:\/\/cinestarslist.eu\/(\d+)\//', $data, $matches)) {
            $this->url = $matches[0];
        }
    }
    ...
    function update() {
        try {
            ...
            if (preg_match_all("/<center><table id=table4(.*?)<center>/ms", $this->fetchedData, $output_array) !== false) {
                foreach ($output_array[0] as $boxnum => $box) {
                    $doc = new DOMDocument();
                    $doc->loadHTML($box,LIBXML_PARSEHUGE  );
